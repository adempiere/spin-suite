...
public class X_I_GLJournal extends PO implements I_I_GLJournal
{
    ...
    public X_I_GLJournal (Context ctx, int I_GLJournal_ID, DB conn)
    {
      super (ctx, I_GLJournal_ID, conn);
      /** if (I_GLJournal_ID == 0)
        {
			setI_GLJournal_ID (0);
    ...
    /** Load Constructor */
    public X_I_GLJournal (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_GLJournal_ID < 1) 
			set_Value (COLUMNNAME_I_GLJournal_ID, null);
		else 
			set_Value (COLUMNNAME_I_GLJournal_ID, Integer.valueOf(I_GLJournal_ID));
	}
