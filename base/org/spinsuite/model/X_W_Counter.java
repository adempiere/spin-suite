...
public class X_W_Counter extends PO implements I_W_Counter
{
    ...
    public X_W_Counter (Context ctx, int W_Counter_ID, DB conn)
    {
      super (ctx, W_Counter_ID, conn);
      /** if (W_Counter_ID == 0)
        {
			setPageURL (null);
			setProcessed (false);
    ...
    /** Load Constructor */
    public X_W_Counter (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (W_Counter_ID < 1) 
			set_Value (COLUMNNAME_W_Counter_ID, null);
		else 
			set_Value (COLUMNNAME_W_Counter_ID, Integer.valueOf(W_Counter_ID));
	}
