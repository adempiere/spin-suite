...
public class X_PP_Order extends PO implements I_PP_Order
{
    ...
    public X_PP_Order (Context ctx, int PP_Order_ID, DB conn)
    {
      super (ctx, PP_Order_ID, conn);
      /** if (PP_Order_ID == 0)
        {
			setAD_Workflow_ID (0);
...
// @#Date@
			...
			setDateStartSchedule (new Date( System.currentTimeMillis() ));
// @#Date@
			setDocAction (null);
// --
			setDocStatus (null);
...
// 100
    ...
    /** Load Constructor */
    public X_PP_Order (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
