...
public class X_M_PromotionReward extends PO implements I_M_PromotionReward
{
    ...
    public X_M_PromotionReward (Context ctx, int M_PromotionReward_ID, DB conn)
    {
      super (ctx, M_PromotionReward_ID, conn);
...
// @SQL=SELECT COALESCE(MAX(SeqNo),0)+10 AS DefaultValue FROM M_PromotionReward WHERE M_Promotion_ID=@M_Promotion_ID@
    ...
    /** Load Constructor */
    public X_M_PromotionReward (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	public void setM_PromotionReward_ID (int M_PromotionReward_ID)
	{
		if (M_PromotionReward_ID < 1) 
			set_Value (COLUMNNAME_M_PromotionReward_ID, null);
		else 
			set_Value (COLUMNNAME_M_PromotionReward_ID, Integer.valueOf(M_PromotionReward_ID));
	}

	/** Get Promotion Reward.
		@return Setup reward for the promotion
	  */
