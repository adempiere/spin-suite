...
public class X_PA_Report extends PO implements I_PA_Report
{
    ...
    public X_PA_Report (Context ctx, int PA_Report_ID, DB conn)
    {
      super (ctx, PA_Report_ID, conn);
      /** if (PA_Report_ID == 0)
        {
    ...
    /** Load Constructor */
    public X_PA_Report (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (PA_Report_ID < 1) 
			set_Value (COLUMNNAME_PA_Report_ID, null);
		else 
			set_Value (COLUMNNAME_PA_Report_ID, Integer.valueOf(PA_Report_ID));
	}
