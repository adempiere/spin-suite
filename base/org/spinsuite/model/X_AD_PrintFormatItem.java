...
public class X_AD_PrintFormatItem extends PO implements I_AD_PrintFormatItem
{
...
// @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM AD_PrintFormatItem WHERE AD_PrintFormat_ID=@AD_PrintFormat_ID@
    ...
    /** Load Constructor */
    public X_AD_PrintFormatItem (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	public void setAD_PrintFormatItem_ID (int AD_PrintFormatItem_ID)
	{
		if (AD_PrintFormatItem_ID < 1) 
			set_Value (COLUMNNAME_AD_PrintFormatItem_ID, null);
		else 
			set_Value (COLUMNNAME_AD_PrintFormatItem_ID, Integer.valueOf(AD_PrintFormatItem_ID));
	}
