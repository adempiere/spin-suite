...
public class X_C_Task extends PO implements I_C_Task
{
    ...
    public X_C_Task (Context ctx, int C_Task_ID, DB conn)
    {
      super (ctx, C_Task_ID, conn);
      /** if (C_Task_ID == 0)
        {
			setC_Phase_ID (0);
			setC_Task_ID (0);
...
// 1
    ...
    /** Load Constructor */
    public X_C_Task (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (C_Task_ID < 1) 
			set_Value (COLUMNNAME_C_Task_ID, null);
		else 
			set_Value (COLUMNNAME_C_Task_ID, Integer.valueOf(C_Task_ID));
	}
