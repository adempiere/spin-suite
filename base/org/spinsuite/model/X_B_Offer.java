...
public class X_B_Offer extends PO implements I_B_Offer
{
    ...
    public X_B_Offer (Context ctx, int B_Offer_ID, DB conn)
    {
      super (ctx, B_Offer_ID, conn);
      /** if (B_Offer_ID == 0)
        {
			setAD_User_ID (0);
			setB_Offer_ID (0);
    ...
    /** Load Constructor */
    public X_B_Offer (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (B_Offer_ID < 1) 
			set_Value (COLUMNNAME_B_Offer_ID, null);
		else 
			set_Value (COLUMNNAME_B_Offer_ID, Integer.valueOf(B_Offer_ID));
	}
