...
public class X_DD_Order extends PO implements I_DD_Order
{
    ...
    public X_DD_Order (Context ctx, int DD_Order_ID, DB conn)
    {
      super (ctx, DD_Order_ID, conn);
      /** if (DD_Order_ID == 0)
        {
			setC_BPartner_ID (0);
...
// 5
    ...
    /** Load Constructor */
    public X_DD_Order (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		return (Date)get_Value(COLUMNNAME_DateReceived);
	}
	...
	{
		return (Date)get_Value(COLUMNNAME_PickDate);
	}
	...
	{
		return (Date)get_Value(COLUMNNAME_ShipDate);
	}
