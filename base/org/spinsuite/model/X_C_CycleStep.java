...
public class X_C_CycleStep extends PO implements I_C_CycleStep
{
    ...
    public X_C_CycleStep (Context ctx, int C_CycleStep_ID, DB conn)
    {
      super (ctx, C_CycleStep_ID, conn);
      /** if (C_CycleStep_ID == 0)
        {
			setC_Cycle_ID (0);
			setC_CycleStep_ID (0);
...
// @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM C_CycleStep WHERE C_Cycle_ID=@C_Cycle_ID@
    ...
    /** Load Constructor */
    public X_C_CycleStep (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (C_Cycle_ID < 1) 
			set_Value (COLUMNNAME_C_Cycle_ID, null);
		else 
			set_Value (COLUMNNAME_C_Cycle_ID, Integer.valueOf(C_Cycle_ID));
	}
