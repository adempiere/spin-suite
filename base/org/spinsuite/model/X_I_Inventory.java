...
public class X_I_Inventory extends PO implements I_I_Inventory
{
    ...
    public X_I_Inventory (Context ctx, int I_Inventory_ID, DB conn)
    {
      super (ctx, I_Inventory_ID, conn);
      /** if (I_Inventory_ID == 0)
        {
			setI_Inventory_ID (0);
    ...
    /** Load Constructor */
    public X_I_Inventory (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_Inventory_ID < 1) 
			set_Value (COLUMNNAME_I_Inventory_ID, null);
		else 
			set_Value (COLUMNNAME_I_Inventory_ID, Integer.valueOf(I_Inventory_ID));
	}
