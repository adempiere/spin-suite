...
public class X_I_Payment extends PO implements I_I_Payment
{
    ...
    public X_I_Payment (Context ctx, int I_Payment_ID, DB conn)
    {
      super (ctx, I_Payment_ID, conn);
      /** if (I_Payment_ID == 0)
        {
    ...
    /** Load Constructor */
    public X_I_Payment (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_Payment_ID < 1) 
			set_Value (COLUMNNAME_I_Payment_ID, null);
		else 
			set_Value (COLUMNNAME_I_Payment_ID, Integer.valueOf(I_Payment_ID));
	}
