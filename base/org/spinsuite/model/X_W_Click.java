...
public class X_W_Click extends PO implements I_W_Click
{
    ...
    public X_W_Click (Context ctx, int W_Click_ID, DB conn)
    {
      super (ctx, W_Click_ID, conn);
      /** if (W_Click_ID == 0)
        {
			setProcessed (false);
			setW_Click_ID (0);
    ...
    /** Load Constructor */
    public X_W_Click (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (W_Click_ID < 1) 
			set_Value (COLUMNNAME_W_Click_ID, null);
		else 
			set_Value (COLUMNNAME_W_Click_ID, Integer.valueOf(W_Click_ID));
	}
