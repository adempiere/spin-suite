...
public class X_ASP_Field extends PO implements I_ASP_Field
{
    ...
    public X_ASP_Field (Context ctx, int ASP_Field_ID, DB conn)
    {
      super (ctx, ASP_Field_ID, conn);
      /** if (ASP_Field_ID == 0)
        {
			setASP_Status (null);
// U
    ...
    /** Load Constructor */
    public X_ASP_Field (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (ASP_Field_ID < 1) 
			set_Value (COLUMNNAME_ASP_Field_ID, null);
		else 
			set_Value (COLUMNNAME_ASP_Field_ID, Integer.valueOf(ASP_Field_ID));
	}
