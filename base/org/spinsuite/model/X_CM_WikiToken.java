...
public class X_CM_WikiToken extends PO implements I_CM_WikiToken
{
    ...
    public X_CM_WikiToken (Context ctx, int CM_WikiToken_ID, DB conn)
    {
      super (ctx, CM_WikiToken_ID, conn);
...
// I
    ...
    /** Load Constructor */
    public X_CM_WikiToken (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	public void setCM_WikiToken_ID (int CM_WikiToken_ID)
	{
		if (CM_WikiToken_ID < 1) 
			set_Value (COLUMNNAME_CM_WikiToken_ID, null);
		else 
			set_Value (COLUMNNAME_CM_WikiToken_ID, Integer.valueOf(CM_WikiToken_ID));
	}
