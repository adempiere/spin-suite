...
public class X_ASP_Task extends PO implements I_ASP_Task
{
    ...
    public X_ASP_Task (Context ctx, int ASP_Task_ID, DB conn)
    {
      super (ctx, ASP_Task_ID, conn);
      /** if (ASP_Task_ID == 0)
        {
			setAD_Task_ID (0);
			setASP_Level_ID (0);
...
// U
    ...
    /** Load Constructor */
    public X_ASP_Task (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (ASP_Task_ID < 1) 
			set_Value (COLUMNNAME_ASP_Task_ID, null);
		else 
			set_Value (COLUMNNAME_ASP_Task_ID, Integer.valueOf(ASP_Task_ID));
	}
