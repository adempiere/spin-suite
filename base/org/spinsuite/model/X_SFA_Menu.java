...
public class X_SFA_Menu extends PO implements I_SFA_Menu, I_Persistent 
{
    ...
    public X_SFA_Menu (Context ctx, int SFA_Menu_ID, DB conn)
    {
      super (ctx, SFA_Menu_ID, conn);
      /** if (SFA_Menu_ID == 0)
        {
			setEntityType (null);
// ECA01
    ...
    /** Load Constructor */
    public X_SFA_Menu (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (SFA_Menu_ID < 1) 
			set_Value (COLUMNNAME_SFA_Menu_ID, null);
		else 
			set_Value (COLUMNNAME_SFA_Menu_ID, Integer.valueOf(SFA_Menu_ID));
	}
