...
public class X_CM_NewsItem extends PO implements I_CM_NewsItem
{
    ...
    public X_CM_NewsItem (Context ctx, int CM_NewsItem_ID, DB conn)
    {
      super (ctx, CM_NewsItem_ID, conn);
      /** if (CM_NewsItem_ID == 0)
        {
    ...
    /** Load Constructor */
    public X_CM_NewsItem (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (CM_NewsItem_ID < 1) 
			set_Value (COLUMNNAME_CM_NewsItem_ID, null);
		else 
			set_Value (COLUMNNAME_CM_NewsItem_ID, Integer.valueOf(CM_NewsItem_ID));
	}
	...
	{
		return (Date)get_Value(COLUMNNAME_PubDate);
	}
