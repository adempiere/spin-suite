...
public class MSFATable extends X_SFA_Table {
	 ...
	 * @param conn
	 */
	public MSFATable(Context ctx, int SFA_Table_ID, DB conn) {
		super(ctx, SFA_Table_ID, conn);
	}
	 ...
	 * @param conn
	 */
	public MSFATable(Context ctx, Cursor rs, DB conn) {
		super(ctx, rs, conn);
	}
	...
	public static Class<?> getClass (Context ctx, String tableName){
		...
		if(className == null
			...
			etmodelpackage = DB.getSQLValueStringEx(ctx, "SELECT et.ModelPackage " +
					...
					"INNER JOIN SFA_Table t ON(t.EntityType = et.EntityType) " +
