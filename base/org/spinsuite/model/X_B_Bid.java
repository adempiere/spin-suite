...
public class X_B_Bid extends PO implements I_B_Bid
{
    ...
    public X_B_Bid (Context ctx, int B_Bid_ID, DB conn)
    {
      super (ctx, B_Bid_ID, conn);
      /** if (B_Bid_ID == 0)
        {
			setAD_User_ID (0);
			setB_Bid_ID (0);
    ...
    /** Load Constructor */
    public X_B_Bid (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (B_Bid_ID < 1) 
			set_Value (COLUMNNAME_B_Bid_ID, null);
		else 
			set_Value (COLUMNNAME_B_Bid_ID, Integer.valueOf(B_Bid_ID));
	}
