...
public class X_I_Invoice extends PO implements I_I_Invoice
{
    ...
    public X_I_Invoice (Context ctx, int I_Invoice_ID, DB conn)
    {
      super (ctx, I_Invoice_ID, conn);
      /** if (I_Invoice_ID == 0)
        {
			setI_Invoice_ID (0);
    ...
    /** Load Constructor */
    public X_I_Invoice (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		return (Date)get_Value(COLUMNNAME_DunningGrace);
	}
	...
	{
		if (I_Invoice_ID < 1) 
			set_Value (COLUMNNAME_I_Invoice_ID, null);
		else 
			set_Value (COLUMNNAME_I_Invoice_ID, Integer.valueOf(I_Invoice_ID));
	}
