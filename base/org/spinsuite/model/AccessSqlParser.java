...
public class AccessSqlParser {
	...
	//		log.fine(m_sqlOriginal);
		...
		//
		LogM.log(Env.getCtx(), getClass(), Level.FINE, toString());
	...
	private void getSelectStatements() {
		...
		try {
			...
			LogM.log(Env.getCtx(), getClass(), Level.SEVERE, m_sqlOriginal, e);
	...
	private TableInfo[] getTableInfo (String sql) {
		...
		if (fromIndex != sql.lastIndexOf(FROM))
			LogM.log(Env.getCtx(), getClass(), Level.SEVERE, "More than one FROM clause - " + sql);
		while (fromIndex != -1) {
			...
			while (index != -1) {
				...
				if (indexClose != -1) {
					...
					LogM.log(Env.getCtx(), getClass(), Level.SEVERE, "Could not remove ON " + from);
					break;
				}			
				index = from.indexOf(ON);
			}
