...
public class X_A_Asset_Disposed extends PO implements I_A_Asset_Disposed
{
    ...
    public X_A_Asset_Disposed (Context ctx, int A_Asset_Disposed_ID, DB conn)
    {
      super (ctx, A_Asset_Disposed_ID, conn);
      /** if (A_Asset_Disposed_ID == 0)
        {
			setA_Asset_Disposed_ID (0);
			setA_Asset_ID (0);
			setA_Disposed_Date (new Date( System.currentTimeMillis() ));
...
// @Date@
    ...
    /** Load Constructor */
    public X_A_Asset_Disposed (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		return (Date)get_Value(COLUMNNAME_A_Disposed_Date);
	}
