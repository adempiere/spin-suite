...
public class X_M_CostQueue extends PO implements I_M_CostQueue
{
    ...
    public X_M_CostQueue (Context ctx, int M_CostQueue_ID, DB conn)
    {
      super (ctx, M_CostQueue_ID, conn);
      /** if (M_CostQueue_ID == 0)
        {
    ...
    /** Load Constructor */
    public X_M_CostQueue (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (M_CostQueue_ID < 1) 
			set_Value (COLUMNNAME_M_CostQueue_ID, null);
		else 
			set_Value (COLUMNNAME_M_CostQueue_ID, Integer.valueOf(M_CostQueue_ID));
	}
