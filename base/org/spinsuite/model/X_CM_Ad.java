...
public class X_CM_Ad extends PO implements I_CM_Ad
{
    ...
    public X_CM_Ad (Context ctx, int CM_Ad_ID, DB conn)
    {
      super (ctx, CM_Ad_ID, conn);
      /** if (CM_Ad_ID == 0)
        {
			setActualClick (0);
    ...
    /** Load Constructor */
    public X_CM_Ad (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (CM_Ad_ID < 1) 
			set_Value (COLUMNNAME_CM_Ad_ID, null);
		else 
			set_Value (COLUMNNAME_CM_Ad_ID, Integer.valueOf(CM_Ad_ID));
	}
