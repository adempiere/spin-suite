...
public class X_S_TimeExpense extends PO implements I_S_TimeExpense
{
    ...
    public X_S_TimeExpense (Context ctx, int S_TimeExpense_ID, DB conn)
    {
      super (ctx, S_TimeExpense_ID, conn);
      /** if (S_TimeExpense_ID == 0)
        {
			setC_BPartner_ID (0);
			setDateReport (new Date( System.currentTimeMillis() ));
// @#Date@
			setDocAction (null);
// CO
			setDocStatus (null);
// DR
    ...
    /** Load Constructor */
    public X_S_TimeExpense (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		return (Date)get_Value(COLUMNNAME_DateReport);
	}
