...
public class X_I_FAJournal extends PO implements I_I_FAJournal
{
    ...
    public X_I_FAJournal (Context ctx, int I_FAJournal_ID, DB conn)
    {
      super (ctx, I_FAJournal_ID, conn);
      /** if (I_FAJournal_ID == 0)
        {
			setI_FAJournal_ID (0);
    ...
    /** Load Constructor */
    public X_I_FAJournal (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_FAJournal_ID < 1) 
			set_Value (COLUMNNAME_I_FAJournal_ID, null);
		else 
