...
public class X_M_PromotionDistribution extends PO implements I_M_PromotionDistribution
{
    ...
    public X_M_PromotionDistribution (Context ctx, int M_PromotionDistribution_ID, DB conn)
    {
      super (ctx, M_PromotionDistribution_ID, conn);
...
// @SQL=SELECT COALESCE(MAX(SeqNo),0)+10 AS DefaultValue FROM M_PromotionDistribution WHERE M_Promotion_ID=@M_Promotion_ID@
    ...
    /** Load Constructor */
    public X_M_PromotionDistribution (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	/** Set Promotion Distribution.
		...
		Distribute order line items into multiple bucket for reward calculation
	  */
	...
	/** Get Promotion Distribution.
		@return Distribute order line items into multiple bucket for reward calculation
	  */
