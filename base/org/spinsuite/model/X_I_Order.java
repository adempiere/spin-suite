...
public class X_I_Order extends PO implements I_I_Order
{
    ...
    public X_I_Order (Context ctx, int I_Order_ID, DB conn)
    {
      super (ctx, I_Order_ID, conn);
      /** if (I_Order_ID == 0)
        {
    ...
    /** Load Constructor */
    public X_I_Order (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_Order_ID < 1) 
			set_Value (COLUMNNAME_I_Order_ID, null);
		else 
			set_Value (COLUMNNAME_I_Order_ID, Integer.valueOf(I_Order_ID));
	}
