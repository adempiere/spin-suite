...
public class X_C_BankStatementLine extends PO implements I_C_BankStatementLine
{
    ...
    public X_C_BankStatementLine (Context ctx, int C_BankStatementLine_ID, DB conn)
    {
      super (ctx, C_BankStatementLine_ID, conn);
...
// @SQL=SELECT COALESCE(MAX(Line),0)+10 FROM C_BankStatementLine WHERE C_BankStatement_ID=@C_BankStatement_ID@
			setProcessed (false);
			setStatementLineDate (new Date( System.currentTimeMillis() ));
// @StatementLineDate@
			setStmtAmt (Env.ZERO);
			setTrxAmt (Env.ZERO);
			setValutaDate (new Date( System.currentTimeMillis() ));
// @StatementDate@
        } */
    }

    /** Load Constructor */
    public X_C_BankStatementLine (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
