...
public class X_K_Topic extends PO implements I_K_Topic
{
    ...
    public X_K_Topic (Context ctx, int K_Topic_ID, DB conn)
    {
      super (ctx, K_Topic_ID, conn);
      /** if (K_Topic_ID == 0)
        {
			setIsPublic (true);
// Y
...
// Y
    ...
    /** Load Constructor */
    public X_K_Topic (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (K_Type_ID < 1) 
			set_Value (COLUMNNAME_K_Type_ID, null);
		else 
			set_Value (COLUMNNAME_K_Type_ID, Integer.valueOf(K_Type_ID));
	}
