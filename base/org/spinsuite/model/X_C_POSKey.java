...
public class X_C_POSKey extends PO implements I_C_POSKey
{
    ...
    public X_C_POSKey (Context ctx, int C_POSKey_ID, DB conn)
    {
      super (ctx, C_POSKey_ID, conn);
      /** if (C_POSKey_ID == 0)
        {
			setC_POSKey_ID (0);
...
// @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM C_POSKey WHERE C_POSKeyLayout_ID=@C_POSKeyLayout_ID@
    ...
    /** Load Constructor */
    public X_C_POSKey (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (C_POSKey_ID < 1) 
			set_Value (COLUMNNAME_C_POSKey_ID, null);
		else 
			set_Value (COLUMNNAME_C_POSKey_ID, Integer.valueOf(C_POSKey_ID));
	}
