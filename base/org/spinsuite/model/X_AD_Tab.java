...
public class X_AD_Tab extends PO implements I_AD_Tab
{
    ...
    public X_AD_Tab (Context ctx, int AD_Tab_ID, DB conn)
    {
      super (ctx, AD_Tab_ID, conn);
      /** if (AD_Tab_ID == 0)
        {
			setAD_Tab_ID (0);
			setAD_Table_ID (0);
...
// @SQL=SELECT COALESCE(MAX(SeqNo),0)+10 AS DefaultValue FROM AD_Tab WHERE AD_Window_ID=@AD_Window_ID@
    ...
    /** Load Constructor */
    public X_AD_Tab (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
