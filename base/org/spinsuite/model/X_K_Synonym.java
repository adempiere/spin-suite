...
public class X_K_Synonym extends PO implements I_K_Synonym
{
    ...
    public X_K_Synonym (Context ctx, int K_Synonym_ID, DB conn)
    {
      super (ctx, K_Synonym_ID, conn);
      /** if (K_Synonym_ID == 0)
        {
			setAD_Language (null);
			setK_Synonym_ID (0);
    ...
    /** Load Constructor */
    public X_K_Synonym (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (K_Synonym_ID < 1) 
			set_Value (COLUMNNAME_K_Synonym_ID, null);
		else 
			set_Value (COLUMNNAME_K_Synonym_ID, Integer.valueOf(K_Synonym_ID));
	}
