...
public class X_AD_Find extends PO implements I_AD_Find
{
    ...
    public X_AD_Find (Context ctx, int AD_Find_ID, DB conn)
    {
      super (ctx, AD_Find_ID, conn);
      /** if (AD_Find_ID == 0)
        {
			setAD_Column_ID (0);
			setAD_Find_ID (0);
...
// ==
    ...
    /** Load Constructor */
    public X_AD_Find (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (AD_Find_ID < 1) 
			set_Value (COLUMNNAME_AD_Find_ID, null);
		else 
			set_Value (COLUMNNAME_AD_Find_ID, Integer.valueOf(AD_Find_ID));
	}
