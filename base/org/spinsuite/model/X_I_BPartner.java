...
public class X_I_BPartner extends PO implements I_I_BPartner
{
    ...
    public X_I_BPartner (Context ctx, int I_BPartner_ID, DB conn)
    {
      super (ctx, I_BPartner_ID, conn);
      /** if (I_BPartner_ID == 0)
        {
			setI_BPartner_ID (0);
...
// N
    ...
    /** Load Constructor */
    public X_I_BPartner (Context ctx, Cursor rs, DB conn)
    {
      super (ctx, rs, conn);
    }
	...
	{
		if (I_BPartner_ID < 1) 
			set_Value (COLUMNNAME_I_BPartner_ID, null);
		else 
			set_Value (COLUMNNAME_I_BPartner_ID, Integer.valueOf(I_BPartner_ID));
	}
