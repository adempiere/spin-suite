 ...
 *
 */
public class RMALineAdapter extends ArrayAdapter<DisplayRMALine> {
	
	/**	Context						*/
	private Context 						ctx;
	/**	Data						*/
	private ArrayList<DisplayRMALine> 		data;
	/**	Decimal Format				*/
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(item == null){
			...
			item = inflater.inflate(R.layout.i_rma_line, null);
		}
		
		//	Get Current Data
		DisplayRMALine mi = data.get(position);
		...
		BigDecimal m_Amt = mi.getQtyReturn().multiply(mi.getPriceEntered());
		
		//	Set Net Line Amount
		TextView tv_Amt = (TextView) item.findViewById(R.id.tv_Amt);
		tv_Amt.setText(m_AmtNumberFormat.format(m_Amt));
	...
	@Override
	public void remove(DisplayRMALine object) {
		data.remove(object);
		notifyDataSetChanged();
	}
}
